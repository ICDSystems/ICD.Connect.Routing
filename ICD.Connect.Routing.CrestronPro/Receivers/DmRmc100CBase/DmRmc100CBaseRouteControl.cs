...
{
	public sealed class DmRmc100CBaseRouteControl<TDevice, TReceiver> : AbstractRouteMidpointControl<TDevice>
		where TDevice : IDmRmc100CBaseAdapter
		where TReceiver : Crestron.SimplSharpPro.DM.Endpoints.Receivers.DmRmc100C
	{
		...
		/// <param name="id"></param>
		public DmRmc100CBaseRouteControl(TDevice parent, int id)
			: base(parent, id)
		{
			Subscribe(parent);
		...
		{
			if(m_Scaler == null)
				throw new InvalidOperationException("The scaler has no instantiated device");
